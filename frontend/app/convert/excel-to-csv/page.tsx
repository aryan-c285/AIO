...
export default function ExcelToCsvPage() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const blob = await uploadFile("/api/convert/excel-to-csv", formData);
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6">
      <div className="animate-fade-in-up">
        <h1 className="text-3xl font-bold text-white">📊 Excel to CSV</h1>
        <p className="mt-2 text-neutral-400">
          Convert your Excel (.xlsx) spreadsheets to CSV format.
        </p>
      </div>
      ...
      <div className="mt-8 space-y-6 animate-fade-in">
        ...
        {loading && <LoadingSpinner message="Converting Excel to CSV…" />}
        ...
        {resultUrl && (
          <DownloadButton blobUrl={resultUrl} filename="converted.csv" />
        )}
      </div>
    </div>
  );
