...
export default function PdfToWordPage() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const blob = await uploadFile("/api/pdf/to-word", formData);
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6">
      ...
      <div className="mt-8 space-y-6 animate-fade-in">
        ...
        {loading && <LoadingSpinner message="Converting PDF to Word…" />}
        ...
        {resultUrl && (
          <DownloadButton blobUrl={resultUrl} filename="converted.docx" />
        )}
      </div>
    </div>
  );
